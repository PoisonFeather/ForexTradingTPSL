...
def _apply_regime_weights(base_w: Dict[str, float], regime: str) -> Dict[str, float]:
    """Amplify weights depending on market regime: 'trend' | 'range' | 'neutral'"""
    w = dict(base_w)
    if regime == "trend":
        for k in ["EMA Crossover + RSI filter", "Donchian Breakout", "ATR Channel Breakout", "Keltner Squeeze", "Trend Pullback to EMA", "MACD Crossover"]:
            if k in w: w[k] *= 1.25
...
def infer_regime(market_features: Dict[str, float]) -> str:
    """
    Simple heuristic:
      - if ADX >= 22 or ATR/price >= 0.0009 -> 'trend'
      - elif ADX <= 16 and ATR/price <= 0.0006 -> 'range'
      - else 'neutral'
    Expect keys: 'adx', 'atr', 'price'
    """
...
def combine_signals(
    ...
    *,
    ...
    signals: list of dicts with keys:
      ...
      - 'conf': float in [0,1]
    base_weights: optional per-strategy weight multiplier
    regime_features: optional {'adx':..., 'atr':..., 'price':...} for dynamic weights
    """
    if not signals:
        return {"dir": "neutral", "confidence": 0.0, "entry": None, "sl": None, "tp": None}

    # base per-strategy weight = 1.0 unless provided
    ...
    for s in signals:
        ...
        conf = max(conf, 0.0)
        # give a tiny floor so very weak but nonzero signals still register
        if 0.0 < conf < min_conf_floor:
            conf = min_conf_floor
        w = base_w.get(name, 1.0) * conf
    ...
    # collect price components with damping for opposing side
    entries, e_w = [], []
    stops, s_w = [], []
    tps, t_w = [], []
    ...
    for s in signals:
        ...
        if s.get("entry") is not None:
            entries.append(float(s["entry"]))
            e_w.append(eff_w)
        if s.get("sl") is not None:
            stops.append(float(s["sl"]))
            s_w.append(eff_w)
        if s.get("tp") is not None:
            tps.append(float(s["tp"]))
            t_w.append(eff_w)
    ...
    if final_dir == "long":
        # prefer a slightly conservative SL (closer to price among the worst-case)
    ...
    # If SL is missing or on the wrong side, synthesize with ATR if provided
    ...
    # basic sanity: enforce SL/TP ordering
    if final_dir == "long" and sl is not None and tp is not None:
        if sl >= entry: sl = min(sl, entry - 0.25 * (tp - entry))
        if tp <= entry: tp = max(tp, entry + (entry - sl) * 1.5)
    if final_dir == "short" and sl is not None and tp is not None:
        if sl <= entry: sl = max(sl, entry + 0.25 * (entry - tp))
        if tp >= entry: tp = min(tp, entry - (sl - entry) * 1.5)
